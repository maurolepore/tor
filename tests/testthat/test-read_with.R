...
test_that("read_with with read.csv lists (file)named dataframes", {
  res <- read_with(utils::read.csv, regexp = "[.]csv$")(readw_example("csv"))
...
test_that("read_with with read_rdata lists (file)named dataframes", {
  ...
  res <- read_with(read_rdata, regexp = "[.]rdata$")(readw_example("rdata"))
...
test_that("read_with reads specific files extention in a mixed directory", {
  expect_is(
    read_with(utils::read.csv, regexp = "[.]csv$")(readw_example("mixed")),
    "list"
  )
})
...
test_that("read_with with zero .rdata file throws informative error", {
  ...
  expect_error(
    read_with(read_rdata, regexp = "[.]rdata$")(readw_example("csv")),
    "Can't find.*rdata"
  )
})
...
test_that("read_with with zero .csv file throws informative error", {
  expect_error(
    read_with(utils::read.csv, "[.]csv")(readw_example("rdata")),
    "Can't find.*csv"
  )
})
